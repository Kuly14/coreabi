...
fn decode_param(param: &ParamType, data: &[u8], offset: usize, validate: bool) -> Result<DecodeResult, Error> {
	match *param {
		ParamType::Address => {
			...
			address.copy_from_slice(&slice[10..]);
...
mod tests {
	...
	fn decode_after_fixed_bytes_with_less_than_32_bytes() {
		let encoded = hex!(
			"
			
		...
		assert_eq!(
			...
			.unwrap(),
			&[
				Token::Address(hex!("11118497afefdc5ac170a664a231f6efb25526ef813f").into()),
