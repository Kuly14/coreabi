...
fn encode_token_append(data: &mut Vec<Word>, token: &Token) {
	match *token {
		Token::Address(ref address) => {
			let mut padded = [0u8; 32];
			padded[10..].copy_from_slice(address.as_ref());
			data.push(padded);
		}
...
mod tests {
	...
	fn encode_address() {
		...
		println!("{:#?}", encoded);
		let expected = hex!("0000000000000000000011111111111111111111111111111111111111111111");
	...
	fn encode_fixed_array_of_dynamic_array_of_addresses() {
		let address1 = Token::Address([0x11u8; 22].into());
		let address2 = Token::Address([0x22u8; 22].into());
		let address3 = Token::Address([0x33u8; 22].into());
		let address4 = Token::Address([0x44u8; 22].into());
	...
	fn encode_fixed_array_of_static_tuples_followed_by_dynamic_type() {
		let tuple1 = Token::Tuple(vec![
			...
			Token::Address([0x44u8; 22].into()),
		]);
		let tuple2 =
			Token::Tuple(vec![Token::Uint(12411.into()), Token::Uint(451.into()), Token::Address([0x22u8; 22].into())]);
	...
	fn encode_params_containing_dynamic_tuple() {
		let address1 = Token::Address([0x22u8; 22].into());
		let bool1 = Token::Bool(true);
		let string1 = Token::String("spaceship".to_owned());
		let string2 = Token::String("cyborg".to_owned());
		let tuple = Token::Tuple(vec![bool1, string1, string2]);
		let address2 = Token::Address([0x33u8; 22].into());
		let address3 = Token::Address([0x44u8; 22].into());
